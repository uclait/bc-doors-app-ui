...
class DateComponent extends Object
{
	...
	static function fromOracleTimestamp($timestamp, $format = 'Y-m-d H:i:s')
	{
		$date = "";
		if (!empty($timestamp))
		{
	        $dateParts = explode(" ", $timestamp);
	        $cnt = sizeof($dateParts);
	        if ($cnt > 1)
	        {
	        	$date = trim($dateParts[0]) . " " . trim($dateParts[1]);
	        	if ($cnt > 2)
	        	{
	        		if (in_array(strtoupper($dateParts[2]), array('AM', 'PM')))
	        			$date .= " " . trim($dateParts[2]);
	        	}
	        	$formattedDate = DateTime::createFromFormat('d-M-y h.i.s.u A', $date);
	        	$date = $formattedDate->format($format);
	        }
		}
