...
class GeneralController extends AppController
{
    ...
    public function getGrouperMembership()
    {
        ...
        $uid = $this->Param->url("uid");
        $use = $this->Param->url("use", 0);
        //1 = dev
        //2 = stage
        //3 = prod
    ...
    public function getTransact()
    {
        ...
        if ($xml)
        {
            ...
            $photoParts = explode(":", $filePath);            if (sizeof($photoParts) > 1)
            {
                ...
                {
                    $errors[] = "`{$filePath} EXISTS for Card Holder `{$uid}`";
                }
            }
            else
            {
                $errors[] = "`{$filePath} invalid file`";
            }
        }
        echo "<pre>";
        print_r($errors);
...
exit(1);
    	...
    	echo "file: " . $file . "\n\n";
        $photoMount = (IS_WINDOWS ? substr(__FILE__, 0, 1) . ":" : "") . $appValues['photo']['download']['mount'];
    	var_dump(file_exists($photoMount . $file));
    	echo "</pre>";
    }
