...
class PhotosController extends AppController
{
    ...
    public function download()
    {
        ...
        if (AUDIT_ACTIVITY) {
            if (DEBUG_WRITE) {
                $this->Debug->write("AUDIT -  Photo Download Start|" . http_build_query($form[$prefixes[0]], '', '|'));
            }
            ;
            $this->Audit->user("Photo Download", $userId, null, http_build_query($form[$prefixes[0]], '', '|'));
            if (DEBUG_WRITE) {
                $this->Debug->write("AUDIT -  Photo Download End");
            }
            ;
        }
        ...
        if ($continue) {
            ...
            if (strlen($uid) <> 9) {
                ...
                if (DEBUG_WRITE) {
                    $this->Debug->write("Start Get Membership");
                }
                ;
                ...
                if (DEBUG_WRITE) {
                    $this->Debug->write("End Get Membership");
                }
                ;
        ...
        if (AUDIT_ACTIVITY) {
            if (sizeof($errors) > 0) {
                if (DEBUG_WRITE) {
                    $this->Debug->write("AUDIT - Photo Download Failed Start|{$userId}|errors=" . sizeof($errors));
                }
                ;
                $this->Audit->user("Photo Download Failed", $userId, null, join(",", $errors));
                if (DEBUG_WRITE) {
                    $this->Debug->write("AUDIT - Photo Download Failed End|{$userId}");
                }
                ;
            } else {
                if (DEBUG_WRITE) {
                    $this->Debug->write("AUDIT - Photo Download Success Start|{$userId}");
                }
                ;
                $this->Audit->user("Photo Download Success", $userId);
                if (DEBUG_WRITE) {
                    $this->Debug->write("AUDIT - Photo Download Success End|{$userId}");
                }
                ;
            }
        }
