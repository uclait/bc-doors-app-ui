...
----------------------------------
*/
define('INI_PATH', '/opt/bruincard');
define('CACHE_NAME_DBASE', 'dbase_connections');
define('CACHE_NAME_APPLICATION', 'application');
define('CACHE_NAME_GROUPER_MERCHANTS', 'Grouper.Merchants');
define('CACHE_NAME_GROUPER_MERCHANT_GROUPS', 'Grouper.Merchants.Groups');

define('IS_WINDOWS', isset($_SERVER['WINDIR']));
...
define('BASE_URL', isset($appValues['base']) ? $appValues['base']['url']['app'] : '/');
define('AUDIT_ACTIVITY', isset($appValues['log_audit_activity']) ? abs($appValues['log_audit_activity']) : false);

define('SEARCH_TIMEOUT', isset($appValues['search']) && isset($appValues['search']['timeout']) ? $appValues['search']['timeout'] : 180);
define('GROUPER_TIMEOUT', isset($appValues['grouper']) && isset($appValues['grouper']['timeout']) ? $appValues['grouper']['timeout'] : 120);
...
function showTime($format = 'H:i:s.u')
{
...
function calculateExecutionSeconds()
{
    ...
    if ($rowCNT > 0)
    {
        ...
        $result = (end($endDate) - end($startDate));
    }

    return $result;
}
function logExecutionTime($label = null)
{
    global $executionTimes;
    $executionTimes[] = showTime() . (empty($label) ? '' : " - {$label}");
}
function displayExecutionTime($return = false)
{
    global $executionTimes;
    $html = '';
    $html = "<pre>";
