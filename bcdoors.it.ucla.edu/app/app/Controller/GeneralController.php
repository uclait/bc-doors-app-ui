...
class GeneralController extends AppController
{
    ...
    public function write_test()
    {
        ...
        $path = "/opt/bruincard/logs/";

        var_dump(file_exists($path));

        $file = $path . "test.log";

        $this->File->write($file, "test", "w");
    ...
    {
        $this->autoRender = false;
        $caPath = "c:/var/www/bcdoors.it.ucla.edu";
        $publicKey = "/var/www/bcdoors.it.ucla.edu/bcdkey.pub";
        $privateKey = "/var/www/bcdoors.it.ucla.edu/bcdkey.pem";
        $url = "https://bcdoors-dev.it.ucla.edu/app/rest/xml/deposits/status?Deposit-start_date=2015-02-01&Deposit-end_date=2015-02-24";

        echo "<pre>\n";
        $opts = array(  CURLOPT_SSH_PUBLIC_KEYFILE => $publicKey,
                        #CURLOPT_SSH_PRIVATE_KEYFILE => $privateKey,
                        #CURLOPT_SSLCERT => $publicKey,
