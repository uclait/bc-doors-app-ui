...
class SystemsController extends AppController
{
    ...
    public function test()
    {
        ...
        //-------------------------------------
        //==> Show Grouper Merchants
        if ($debug) {logExecutionTime("Show Grouper Merchants Start");};
        $values = '';
        $cacheValues = Cache::read(CACHE_NAME_GROUPER_MERCHANTS);
        self::recursive($cacheValues, $values);
        $this->set('grouper_merchants', $values);
        if ($debug) {logExecutionTime("Show Grouper Merchants End");};
        //-------------------------------------
        //==> Show Grouper Merchants Groups
        if ($debug) {logExecutionTime("Show Grouper Merchants Groups Start");};
        $values = '';
        $cacheValues = Cache::read(CACHE_NAME_GROUPER_MERCHANT_GROUPS);
        self::recursive($cacheValues, $values);
        $this->set('grouper_merchant_groups', $values);
        if ($debug) {logExecutionTime("Show Grouper Merchants Groups End");};
        //-------------------------------------
        //==> Cached Files
