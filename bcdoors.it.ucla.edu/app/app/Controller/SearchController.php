...
class SearchController extends AppController 
{
    ...
    public function json_card_holder()
    {
        ...
        if (AUDIT_ACTIVITY)
        {
            if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find BruinCard Holder Start|" . http_build_query($form[$prefixes[0]], '', '|'));};
            $this->Audit->user("Find BruinCard Holder", $userId, null, http_build_query($form[$prefixes[0]], '', '|'));
            if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find BruinCard Holder End");};
        }
        ...
        if ($continue)
        {
            ...
            {
                if (DEBUG_WRITE) {$this->Debug->write("Get Cached API Start");};
                $cardHolders = $this->CacheObject->get($cacheName);
                if (DEBUG_WRITE) {$this->Debug->write("Get Cached API End");};
            }
            else
            {
                ...
                if ($this->Http->status == $this->Http->STATUS_CODE_OK)
                {
                    ...
                    if ($xml)
                    {
                        if (DEBUG_WRITE) {$this->Debug->write("Generate Results Start");};
                        ...
                        if (DEBUG_WRITE) {$this->Debug->write("Generate Results End");};
                    }
                }
            }
        ...
        if (AUDIT_ACTIVITY)
        {
            if (sizeof($errors) > 0)
            {
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find BruinCard Holder Failed Start|errors=" . sizeof($errors));};
                $this->Audit->user("Find BruinCard Holder Failed", $userId, null, http_build_query($form[$prefixes[0]], '', '|') . "|errors=" . sizeof($errors));                
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT- Find BruinCard Holder Failed End");};
            }
            else
            {
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find BruinCard Holder Results|results=" . sizeof($cardHolders));};
                $this->Audit->user("Find BruinCard Holder Results", $userId, null, http_build_query($form[$prefixes[0]], '', '|') . "|results=" . sizeof($cardHolders));                
    ...
    public function json_door_plan() 
    {
        ...
        if (AUDIT_ACTIVITY)
        {
            if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find Door Access Plan Start|" . http_build_query($form[$prefixes[0]], '', '|'));};
            $this->Audit->user("Find Door Access Plan", $userId, null, http_build_query($form[$prefixes[0]], '', '|'));
            if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find Door Access Plan End");};
        }
        ...
        if ($continue)
        {
            ...
            if (DEBUG_WRITE) {$this->Debug->write("Grouper API Start");};
            $groups = $this->GrouperApi->loadGroups();
            if (DEBUG_WRITE) {$this->Debug->write("Grouper API End");};
        ...
        if (AUDIT_ACTIVITY)
        {
            if (sizeof($errors) > 0)
            {
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find Door Access Plan Failed Start|errors=" . sizeof($errors));};
                $this->Audit->user("Find Door Access Plan Failed", $userId, null, http_build_query($form[$prefixes[0]], '', '|') . "|errors=" . sizeof($errors));                
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find BruinCard Holder Failed End");};
            }
            else
            {
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find Door Access Plan Results|results=" . sizeof($results));};
                $this->Audit->user("Find Door Access Plan Results", $userId, null, http_build_query($form[$prefixes[0]], '', '|') . "|results=" . sizeof($results));                
                if (DEBUG_WRITE) {$this->Debug->write("AUDIT - Find Door Access Plan Results End");};
            }
        }
