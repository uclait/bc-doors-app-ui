...
class DoorPlansController extends AppController 
{
    ...
    //==> Removed by RRD on 04/06/2015
    //==> , 'CardHolderPPID'
    var $components = array('AccessPlan', 'Breadcrumbs', 'Validate');
    ...
    public function index()
    {
        ...
        if ($continue)
        {
            ...
            if (DEBUG_WRITE) {$this->Debug->write("Start Get Allow Door Access `{$doorPlanId}`");};
            $accessPlan = $this->GrouperApi->allowDoorAccess($this->Session->read('merchants'), $doorPlanId);
            if (DEBUG_WRITE) {$this->Debug->write("End Get Allow Door Access `{$doorPlanId}`");};
            ...
            if ($continue)
            {
                ...
                if (DEBUG_WRITE) {$this->Debug->write("Start Get Pending Card Holders");};
                $data = $this->PendingAccessPlan->find("all", array("conditions" => $conditions, 'order' => "id"));
                if (DEBUG_WRITE) {$this->Debug->write("End Get Pending Card Holders");};
                $rowCNT = sizeof($data);
                ...
                {
                    if (DEBUG_WRITE) {$this->Debug->write("Start Get Pending Card Holder Access Plan `{$uid}`");};
                    $data = $this->AccessPlan->retrieve($uid, $this->Array->extractValues($values, "path"));
                    if (DEBUG_WRITE) {$this->Debug->write("End Get Pending Card Holder Access Plan `{$uid}`");};
                    $path = $values[0]['path'];
                    if (isset($data[$path]))
                    {
                        ...
                        {
                            if (DEBUG_WRITE) {$this->Debug->write("Start Delete Pending Card Holder Access Plan `{$uid}`");};
                            $fields = array("UCLA_UID" => $uid, "PATH" => $path);
                            $result = $this->PendingAccessPlan->deleteAll($fields);
                            if (DEBUG_WRITE) {$this->Debug->write("End Delete Pending Card Holder Access Plan `{$uid}`");};
                            unset($pending[$uid]);
                        }
                    }
                ...
                if (DEBUG_WRITE) {$this->Debug->write("Start Get Access Plan Members `{$doorPlanId}`");};
                $data = $this->GrouperApi->getMembers($doorPlanId);
                if (DEBUG_WRITE) {$this->Debug->write("End Get Access Plan Members `{$doorPlanId}`");};
            }
        }
        else
        {
