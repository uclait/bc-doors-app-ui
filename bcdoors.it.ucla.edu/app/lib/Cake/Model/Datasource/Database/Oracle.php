...
/**
 * The Model that called this Datasource
 * @var Model
 */
	public $_model = null;
 ...
 * Depending on the type of query, OCI offset function and OCI fetch all's skip variable is required, if the query is unoptimized
 * If it is optimized, there is no need for it's special functions.
 * False = no need
 * True = need
 * @var boolean
 */
	public $__requireOCIOffset = false;
 ...
 */
	function connect()
    {
		...
		if ($this->connection) {
			...
			//$this->execute("ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH:MI:SS'");
 ...
 */
	public function disconnect() {
		if ($this->connection) {
			...
			/*DEBUG*/ //debug($this->connection, false, true);
			/*DEBUG*/ //debug($this->connected, false, true);
			return !$this->connected;
		}
	}
 ...
 */
	function _scrapeSQL($sql)
	{
		...
		//$this->_map[] = array("__Oracle", "ROWNUM");
		/*DEBUG*/ //
        //pr($this->_map);
	}
 ...
 */
	...
	function _execute($sql, $params = array(), $prepareOptions = array())
	{
		...
		if ($this->__requireOCIOffset) {
			...
			$this->_numRows = oci_fetch_all($this->_statementId, $this->_results, 0, -1, OCI_NUM | OCI_FETCHSTATEMENT_BY_ROW);
		}
            ...
            foreach($this->_results[$this->_currentRow] as $index => $field)
            {
                if (isset($this->_map[$index]))
                {
                    ...
                    else
                    {
                            ...
                            $resultRow[$table][strtolower($column)] = $this->_results[$this->_currentRow][$index];
                    }
                }
                else
                {
                    //$resultRow[$index][$field] = $this->_results[$this->_currentRow][$index];
                    $resultRow[] = $this->_results[$this->_currentRow][$index];
                }
            }
            $this->_currentRow++;
 ...
 */
	function listSources($data = NULL)
        ...
        $sql = 'SELECT table_name AS name FROM all_tables order by table_name';
	    $sql = "SELECT table_name AS name FROM all_tables WHERE tablespace_name <> 'SYSAUX' order by table_name";
		#echo "start table: " . showTime() . "<BR>";
		if (!$this->execute($sql))
                {
			return false;
		}
		#echo $sql . "<br>"; 
		#echo "end table: " . showTime() . "<BR>";
 ...
 *
 * This overload function purpose is only to get the model into a class variable
 *
 ...
 */
	public function describe($model) {
		...
		$sql = "SELECT all_tab_columns.COLUMN_NAME, all_tab_columns.DATA_TYPE, all_tab_columns.DATA_LENGTH, all_tab_columns.OWNER, all_constraints.CONSTRAINT_TYPE ";
		$sql .= "FROM all_tab_columns ";
		$sql .= "LEFT OUTER JOIN all_cons_columns on all_tab_columns.column_name = all_cons_columns.column_name AND all_cons_columns.table_name = all_tab_columns.table_name ";
		$sql .= "LEFT OUTER JOIN all_constraints on all_constraints.constraint_name = all_cons_columns.constraint_name ";
		$sql .= "WHERE all_tab_columns.table_name = '".strtoupper($this->fullTableName($model))."' ";
		$sql .= "ORDER BY all_tab_columns.COLUMN_ID";
		//$sql = 'SELECT COLUMN_NAME, DATA_TYPE, DATA_LENGTH, CONSTRAINT_TYPE, OWNER FROM all_constraints, all_tab_columns WHERE table_name = \''.strtoupper($this->fullTableName($model)) . '\' ORDER BY COLUMN_ID';
		/*DEBUG*/ //pr($sql);
		...
		$fields = array();
		while ($this->_currentRow < $this->_numRows) {
			foreach ($this->_results[$this->_currentRow] as $index => $field) {
				$tempData[$map[$index][0]][$map[$index][1]] = $field;
			}
			foreach ($tempData as $table => $data) {
				foreach ($data as $col => $value) {
					$fields[$tempData["all_tab_columns"]["COLUMN_NAME"]][strtolower($col)] = $value;
				}
			}
			$this->_currentRow++;
		}
 ...
 */
	function renderStatement($type, $data) {
		...
		switch (strtolower($type)) {
			case 'select':
				$this->_scrapeSQL("SELECT {$fields} FROM {$table}");
				if ($fields == "COUNT(*) AS count" || !empty($joins)) {
					//This query requires OCI's offset and skip function
					$this->__requireOCIOffset = true;
					return "SELECT {$fields} FROM {$table} {$alias} {$joins} {$conditions} {$group} {$order} {$limit}";
				} else {
					//This query does not need OCI's offset and skip function, it's already optimized
					$this->__requireOCIOffset = false;
					if ($this->_offset == 0 && $this->_limit == 0) {
						$limitOffset = "";
					} else {
						$limitOffset = "WHERE raw_rnum_ between ".($this->_offset+1)." and ".($this->_offset+$this->_limit);
					}
					$sql = "SELECT * FROM (SELECT raw_sql_.*, rownum raw_rnum_ FROM (SELECT {$fields} FROM {$table} {$alias} {$joins} {$conditions} {$group} {$order} {$limit}) raw_sql_ ) ".$limitOffset;
					/*DEBUG*/ //pr($sql);
					return $sql;
				}
			break;
		...
		if ($query = $this->generateAssociationQuery($model, $linkModel, $type, $association, $assocData, $queryData, $external, $resultSet)) {
			...
			if ($type === 'hasMany' && (!isset($assocData['limit']) || empty($assocData['limit']))) {
				...
				if (!empty($ins)) {
					...
					/*DEBUG*/ //pr($query);
					$q = $query;
					$res = $this->fetchAll($query, $model->cacheQueries, $model->alias);
